...
data
  ActivationCache
    ...
    a
  where
  Tokens :: ActivationCache device dtype batchSize inputSeqLen dmodel nheads nlayers dvocab (Tensor device dtype '[inputSeqLen])
  PosEmbed :: ActivationCache device dtype batchSize inputSeqLen dmodel nheads nlayers dvocab (Tensor device dtype '[inputSeqLen, dmodel])
  Embed :: ActivationCache device dtype batchSize inputSeqLen dmodel nheads nlayers dvocab (Tensor device dtype '[inputSeqLen, dmodel])
  -- START: Components that live inside of transformer blocks, hence, they have an additional layer index
  Attention :: ActivationCache device dtype batchSize inputSeqLen dmodel nheads nlayers dvocab (Tensor device dtype '[nlayers, batchSize, numHeads, inputSeqLen, inputSeqLen])
  -- END
  Logits :: ActivationCache device dtype batchSize inputSeqLen dmodel nheads nlayers dvocab (Tensor device dtype '[inputSeqLen, dvocab])
...
type GPT2ActivationMap device batchSize inputSeqLen = DMap (ActivationCache device D.Float batchSize inputSeqLen 768 12 12 50258) Identity
...
transformerLM ::
  ...
  (Tensor device dtype '[batchSize, inputSeqLen, vocabSize], GPT2ActivationMap device batchSize inputSeqLen)
